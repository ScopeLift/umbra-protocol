...
export default boot(({ Vue }) => {
  ...
  // `info` is a Vue-specific error info, e.g. which lifecycle hook the error was found in. Only
  // available in 2.2.0+
  ...
  window.addEventListener('unhandledrejection', function (event) {
    ...
    throw new Error(event.reason); // convert promise rejection to ordinary JS error
  });
});
