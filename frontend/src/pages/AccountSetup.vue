...
function useKeys() {
  ...
  async function publishKeys() {
    ...
    try {
      ...
      const response = await domainService.value.setPublicKeys(
        selectedName.value,
      ...
      if (ens.isEnsDomain(selectedName.value)) {
        // ENS just returns one transaction
        const tx = response as TransactionResponse;
        txNotify(tx.hash);
        await tx.wait();
      } else {
        // CNS requires two transactions
        const { txSpending, txViewing } = response as {
          txSpending: TransactionResponse;
          txViewing: TransactionResponse;
        };
        txNotify(txSpending.hash);
        txNotify(txViewing.hash);
        await txSpending.wait();
        await txViewing.wait();
      }
