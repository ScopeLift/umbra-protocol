...
describe('Umbra class', () => {
  ...
  before(async () => {
    ...
    sender.connect(hre.ethers.provider);
    receiver = ethers.Wallet.fromMnemonic(mnemonic as string, `${path as string}/${receiverIndex}`);
    receiver.connect(hre.ethers.provider);

    // Load other signers
    deployer = (await hre.ethers.getSigners())[0]; // used for deploying contracts
  });
  ...
  describe('Send, scan, and withdraw funds', () => {
    beforeEach(() => {
      ...
      sender = sender.connect(hre.ethers.provider);
    });
