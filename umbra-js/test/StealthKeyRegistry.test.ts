...
describe('StealthKeyRegistry class', () => {
  ...
  it('setStealthKeys: throws if registry has no signer and a signer is not specified', async () => {
    ...
    const errorMsg = 'sending a transaction requires a signer (operation="sendTransaction", code=UNSUPPORTED_OPERATION, version=contracts/5.1.0)'; // prettier-ignore
  ...
  it('sets and gets stealth keys', async () => {
    ...
    const umbra = new Umbra(ethers.provider, 4);
